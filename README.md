...
- 🎫 [IT Service Desk Insights Agent](./01-scenarios/IT-Service-Desk-Insights-Agent/) — Read-only Agent Builder scenario for ServiceNow knowledge and catalog answers in Microsoft 365 Copilot, with permission trimming, citations, and consistency checks.
- 🎫 [IT Service Desk Insights Agent (GHCP)](./01-scenarios/IT-Service-Desk-Insights-Agent-ghcp/) — Separate GitHub Copilot harness scenario inside Copilot Studio for ServiceNow, Nexthink diagnostics, and Dynamics Customer Service handoff. Includes a capability matrix and 17 skill definitions; routing stays backend-owned and closure is requester-only.
