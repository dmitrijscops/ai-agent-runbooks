...
- 🎫 [IT Service Desk Insights Agent](./01-scenarios/IT-Service-Desk-Insights-Agent/) — Read-only knowledge reference plus a Copilot Studio GitHub Copilot harness blueprint for ServiceNow, Nexthink diagnostics, and Dynamics Customer Service handoff. Agent-scoped capabilities control actions; routing stays backend-owned and closure is requester-only.
