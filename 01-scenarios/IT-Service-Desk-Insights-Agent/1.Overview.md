...
**Primary Tools**: ServiceNow, Nexthink APIs, Microsoft Dataverse for Dynamics 365 Customer Service, agent-scoped skills and workflows
**Complexity**: Intermediate for knowledge-only; advanced for investigation and controlled actions
**Status**: Documentation blueprint available; tenant-specific skills, tools, and workflows must be implemented
...
**Closure is requester-only:** the authenticated requester must explicitly ask to close their
...
1. Identify the authenticated requester, affected service/device, and explicit time window.
...
- **Administrative, technician-initiated, or autonomous closure.** Read/update access is not
  requester identity. Closing one record never implicitly closes a linked record.
