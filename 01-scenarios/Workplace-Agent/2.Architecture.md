...
sequenceDiagram
    ...
    participant DV as Dataverse<br/>Support Ticket
...
sequenceDiagram
    ...
    participant DV as Dataverse<br/>License Request
...
### Support Ticket (`ms_supportticket`)

| Display name | Schema name | Data type | Required | Notes |
|--------------|-------------|-----------|----------|-------|
| Ticket number | `ms_ticketnumber` | Autonumber | Primary name | Format such as `CS-{SEQNUM:0000}`. |
| Ticket Status | `ms_status` | Choice | Yes | New / In Progress / Waiting for Response / Resolved / Closed. |
| Inquiry category | `ms_category` | Choice | Yes | HR / IT / Legal / Other; can support routing. |
| Response date | `ms_responsedate` | Date only | No | Date the final answer was recorded. |
| Inquiry body | `ms_inquirybody` | Multiline text | Yes | User's original question. |
| AI-generated answer | `ms_aianswer` | Multiline text | No | Agent-generated answer captured at ticket creation. |
| Answer | `ms_answer` | Multiline text | No | Final answer entered by the administrator. |
| Assignee | `ms_assignee` | Lookup | No | User or team responsible for handling the ticket. |
| Publish status | `ms_publishstatus` | Choice | No | Unpublished / Published. |
| Published on | `ms_publishedon` | Date and time | No | Set after successful FAQ publishing. |

### License Request (`ms_licenserequest`)

| Display name | Schema name | Data type | Required | Notes |
|--------------|-------------|-----------|----------|-------|
| Request number | `ms_requestnumber` | Autonumber | Primary name | Format such as `LICN-{SEQNUM:0000}`. |
| Full name | `ms_fullname` | Single line text | Yes | Requester's full name. |
| Email | `ms_email` | Single line text | Yes | Requester's email address. |
...
| Question | `ms_question` | Multiple lines of text | No | User-facing FAQ question. |
| Answer | `ms_answer` | Multiple lines of text | No | Approved answer. |
| Category | `ms_category` | Choice | Yes | HR / Legal / IT / Other. |
