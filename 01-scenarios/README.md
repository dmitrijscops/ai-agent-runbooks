...
| [IT-Service-Desk-Insights-Agent](./IT-Service-Desk-Insights-Agent/) | IT Service Management / Employee Support | Read-only knowledge reference plus an investigation/resolution blueprint with agent-scoped capabilities for ServiceNow, Nexthink, and Dynamics Customer Service. Backend-owned routing; requester-only closure. | Copilot Studio GitHub Copilot harness, ServiceNow, Nexthink APIs, Dataverse; Agent Builder reference | Documentation available; tenant implementation required |
